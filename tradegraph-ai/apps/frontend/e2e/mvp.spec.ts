...
async function mockApi(page: Page, role: "user" | "admin" = "user") {
  ...
  await page.route("http://localhost:8000/api/v1/**", async (route) => {
    ...
    if (path.endsWith("/products") || path.includes("/products?")) return fulfill({ count: 1, next: null, previous: null, results: [{ id: 1, code: "01", level: 2, name: "Live animals", parent_code: null, is_active: true }] })
    if (path.endsWith("/products/01")) return fulfill({ id: 1, code: "01", level: 2, name: "Live animals", parent_code: null, is_active: true })
    if (path.endsWith("/data-sources")) return fulfill({ data: [{ code: "BACI", name: "CEPII BACI", description: "Harmonized bilateral trade data.", homepage_url: "https://www.cepii.fr/", attribution_text: "CEPII BACI", requires_api_key: false, is_enabled: true, created_at: "2024-01-01T00:00:00Z", updated_at: "2024-01-01T00:00:00Z" }], meta })
    ...
    if (path.endsWith("/analytics/country-profile/UZB")) return fulfill({ data: { iso3: "UZB", total_imports_usd: 1000000, total_exports_usd: 2000000, top_products: [{ code: "01", trade_value_usd: 1000000 }], top_suppliers: [], top_destinations: [], concentration: { hhi: 0.5, supplier_count: 2, suppliers: [] }, exposure: { score: 42, components: {}, component_explanations: {}, insufficient_history: false, quantity_data_available: true, methodology: "Transparent indicator.", hhi: 0.5, supplier_count: 2, volatility: 0.1 }, history: [{ year: 2024, value: 3000000, quantity: 100 }] }, meta })
    if (path.endsWith("/analytics/product-profile/01")) return fulfill({ data: { hs2: "01", name: "Live animals", global_trend: [{ year: 2024, value: 3000000, quantity: 100 }], top_exporters: [], top_importers: [], fastest_growing_countries: [], concentration: { hhi: 0.5, supplier_count: 2, suppliers: [] }, anomalies: [] }, meta })
    if (path.endsWith("/analytics/map")) return fulfill({ data: [{ exporter: { iso3: "UZB", name: "Uzbekistan", latitude: 41.3, longitude: 64.5 }, importer: { iso3: "USA", name: "United States", latitude: 38, longitude: -97 }, trade_value_usd: 1000000 }], meta })
    if (path.endsWith("/ml/forecast")) return fulfill({ request_id: "22222222-2222-4222-8222-222222222222", historical_values: [{ year: 2023, value: 900000 }, { year: 2024, value: 1000000 }], forecast: { year: 2025, value: 1100000, lower_bound: 700000, upper_bound: 1500000, coverage_level: 0.95, interval_method: "validation-residual normal approximation" }, baseline_forecast: 950000, model_name: "three_year_moving_average", model_version: "baseline-v1", dataset_version: "sample-v1", training_period: {}, metrics: {}, main_input_factors: ["trade_value_lag_1"], factor_definitions: [{ feature: "trade_value_lag_1", display_name: "Recent trade value", description: "Previous year.", unit: "USD", direction: "Higher may increase.", limitation: "Cannot know shocks." }], explanations: ["The baseline follows recent trade."], warnings: [], used_fallback: true, data_freshness: 2024, lineage: { data_source: "BACI", dataset_version: "sample-v1", feature_dataset_version: "sample-v1", feature_schema_version: "forecast-v2", model_version: "baseline-v1", training_period: {}, inference_timestamp: "2024-01-01T00:00:00Z" } })
    if (path.endsWith("/ml/supplier-recommendations")) return fulfill({ candidates: [{ country: "UZB", name: "Uzbekistan", recommendation_score: 82, component_scores: { trade_scale: 0.9, stability: 0.8, diversification: 0.7 }, reasons: ["Strong recent trade scale", "Relatively stable supply history"] }], data_freshness: 2024, dataset_version: "sample-v1", methodology: "Transparent weighted ranking." })
    if (path.endsWith("/admin/data-health")) return fulfill({ sources: [], active_dataset: { version: "sample-v1", source: "BACI", row_count: 1000, period_start: 2020, period_end: 2024, promoted_at: "2024-01-01T00:00:00Z" }, versions: [{ version: "sample-v1", source: "BACI", status: "active", is_active: true, row_count: 1000, period_start: 2020, period_end: 2024 }], ingestion_runs: [], failures: 0, models: [], active_models: 1, data_freshness: 2024, cache_status: "healthy" })
...
test("authenticated dashboard navigation and core interactions", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Log in" }).click()
  await expect(page.getByRole("heading", { name: "Overview" })).toBeVisible()
  await expect(page.getByRole("img", { name: "Illustrative 30-day portfolio performance chart" })).toBeVisible()
  await page.getByRole("searchbox", { name: "Search dashboard" }).fill("Portfolio")
  await page.getByRole("searchbox", { name: "Search dashboard" }).press("Enter")
  ...
  await expect(page.getByRole("img", { name: "NVDA illustrative 1Y price chart" })).toBeVisible()
  ...
  await expect(page.getByRole("button", { name: "Enable two-factor authentication" })).toBeDisabled()
})
...
test("responsive, dark mode, URL filters, and reference screenshots", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Log in" }).click()
  await expect(page.getByRole("heading", { name: "Overview" })).toBeVisible()
  await page.screenshot({ path: join(screenshotDir, "dashboard-desktop.png"), fullPage: true })
  await page.setViewportSize({ width: 390, height: 844 })
  await page.getByRole("button", { name: "Open dashboard navigation" }).click()
  await expect(page.getByRole("dialog")).toBeVisible()
  ...
  await page.screenshot({ path: join(screenshotDir, "explorer-desktop.png"), fullPage: true })

  await page.goto("/countries/UZB")
  await expect(page.getByRole("heading", { name: "Uzbekistan" })).toBeVisible()
  await page.screenshot({ path: join(screenshotDir, "country-profile.png"), fullPage: true })

  await page.goto("/products/01")
  await expect(page.getByRole("heading", { name: "Live animals" })).toBeVisible()
  await page.screenshot({ path: join(screenshotDir, "product-profile.png"), fullPage: true })

  await page.goto("/forecast")
  await page.getByRole("button", { name: "Run forecast" }).click()
  await expect(page.getByText("Active-model forecast")).toBeVisible()
  await page.screenshot({ path: join(screenshotDir, "forecast.png"), fullPage: true })
  ...
  await page.screenshot({ path: join(screenshotDir, "supplier-finder.png"), fullPage: true })

  await page.goto("/map")
  await expect(page.getByRole("region", { name: "Trade flow visualization" })).toBeVisible()
  await page.screenshot({ path: join(screenshotDir, "trade-map.png"), fullPage: true })
  ...
  await page.screenshot({ path: join(screenshotDir, "admin-data-health.png"), fullPage: true })
