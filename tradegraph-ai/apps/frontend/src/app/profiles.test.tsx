...
import { CountryAnalyticsProfile, ProductAnalyticsProfile } from "@/components/analytics-profiles"

describe("analytics profile routes", () => {
  it("passes the ISO3 segment to the country profile", async () => {
    const page = await CountryPage({ params: Promise.resolve({ iso3: "UZB" }) })
  ...
  it("passes leading-zero product codes to the product profile", async () => {
    const page = await ProductPage({ params: Promise.resolve({ code: "010121" }) })
