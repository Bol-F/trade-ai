...
export function CountryDetail({ iso3 }: { iso3: string }) {
  const query = useQuery({
    queryKey: queryKeys.country(iso3),
    queryFn: () => catalogApi.country(iso3),
  });
  ...
  return (
    <PageContainer className="py-12 md:py-16">
      ...
      <p className="mt-10 font-mono text-sm text-primary">
        {country.iso3} · M49 {country.m49_code}
      </p>
      <h1 className="mt-3 text-4xl font-semibold">{country.name}</h1>
  ...
  return (
    <PageContainer className="py-12 md:py-16">
      ...
      <p className="mt-10 font-mono text-sm text-primary">
        ...
        {product.classification.version} · HS{product.level}
      </p>
      <h1 className="mt-3 font-mono text-4xl font-semibold">{product.code}</h1>
      <p className="mt-5 max-w-3xl text-xl leading-8">{product.name}</p>
      <div className="mt-10 rounded-xl border bg-card p-6">
        <p className="text-xs uppercase tracking-wide text-muted-foreground">
          Description
        </p>
        <p className="mt-3 leading-7 text-muted-foreground">
          {product.description || "No additional description is available."}
        </p>
        {product.parent_code && (
          <p className="mt-6 text-sm">
            ...
            <Link
              ...
              href={`/products/${product.parent_code}`}
            >
              {product.parent_code}
            </Link>
          </p>
...
function DetailLoading() {
  return (
    <PageContainer className="py-16">
      ...
      <div className="mt-6 h-12 w-80 animate-pulse rounded bg-muted" />
      <div className="mt-10 h-48 animate-pulse rounded-xl bg-muted" />
    </PageContainer>
  );
}
function DetailMissing({ back }: { back: string }) {
  return (
    <PageContainer className="py-20 text-center">
      <h1 className="text-2xl font-semibold">Record not found</h1>
      <Link href={back} className="mt-4 inline-block underline">
        Return to catalog
      </Link>
    </PageContainer>
  );
