...
export function AnomaliesDashboard() {
  ...
  const context = (query.data?.data ?? []).filter(
    (row) =>
      selectedYear === undefined || Math.abs(row.year - selectedYear) <= 2,
  );
  ...
  return (
    <PageContainer className="py-10">
      ...
      <p className="mt-3 max-w-3xl text-muted-foreground">
        MAD-based robust z-scores, rolling deviation, supplier disappearance,
        unit-value changes and quantity changes.
      </p>
      <div className="mt-8 grid gap-4 rounded-xl border bg-card p-4 sm:grid-cols-3">
        <Label>
          ...
          <Input
            ...
            placeholder="ISO3"
            value={importer}
            onChange={(event) => setImporter(event.target.value.toUpperCase())}
          />
        </Label>
        <Label>
          HS2 product
        ...
        <Label>
          ...
          >
            ...
            <option value="high_anomaly">High anomaly</option>
          </Select>
        </Label>
      </div>
      ...
      {selected && (
        <section className="mt-6 rounded-xl border bg-card p-5">
          <h2 className="font-semibold">Context around {selected.year}</h2>
          <EChart
            option={{
              xAxis: { type: "category", data: context.map((row) => row.year) },
              yAxis: { type: "value" },
              tooltip: { trigger: "axis" },
              series: [
                { type: "bar", data: context.map((row) => row.anomaly_score) },
              ],
            }}
          />
        </section>
