...
export function SignalsDashboard() {
  ...
  return (
    <>
      ...
      <Card className="shadow-none">
        <CardContent className="p-4">
          <div className="grid gap-3 md:grid-cols-[minmax(12rem,1fr)_repeat(3,minmax(9rem,.35fr))]">
            ...
            <label>
              <span className="sr-only">Signal type</span>
              <Select
                value={type}
                onChange={(event) => {
                  setType(event.target.value as SignalType | "All");
                  resetPage();
                }}
              >
                <option>All</option>
            ...
            <label>
              ...
              <Select
                value={sort}
                onChange={(event) =>
                  setSort(event.target.value as "confidence" | "recent")
                }
              >
      ...
      <div
        role="note"
        className="mt-4 flex gap-3 rounded-xl border border-warning/30 bg-warning-surface p-4 text-sm"
      >
        <ShieldAlert
          aria-hidden="true"
        ...
        />
        <p>
          <strong>Analytical signal—not financial advice.</strong> Signals use
          illustrative demo data, carry uncertainty, and do not guarantee price
          direction or performance.
        </p>
      </div>
      <Card className="mt-4 shadow-none">
        {visible.length ? (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Risk / horizon</TableHead>
              ...
              <TableBody>
                {visible.map((signal) => (
                  <TableRow key={signal.id} tabIndex={0}>
                    <TableCell className="pl-5">
                      <p className="font-medium">{signal.asset}</p>
                      <p className="font-mono text-xs text-muted-foreground">
                        {signal.ticker} · {signal.createdAt}
                      </p>
                    </TableCell>
                    <TableCell>
                      <StatusBadge tone={signalTone(signal.type)}>
                        {signal.type}
                      </StatusBadge>
                    </TableCell>
                    <TableCell
          ...
          <div className="p-10 text-center">
            ...
            />
            <h2 className="mt-4 font-medium">No matching signals</h2>
            <p className="mt-2 text-sm text-muted-foreground">
              Adjust the search or filters to review other demo signals.
            </p>
          </div>
        )}
      </Card>
        ...
        <DialogContent>
          {selected && (
            <>
              <DialogHeader>
                <DialogTitle>
                  {selected.asset} · {selected.ticker}
                </DialogTitle>
                <DialogDescription>
                  Created {selected.createdAt} · {selected.horizon} horizon
                </DialogDescription>
              </DialogHeader>
              <div className="flex flex-wrap gap-2">
                <StatusBadge tone={signalTone(selected.type)}>
                  {selected.type}
                </StatusBadge>
                <Badge variant="outline">{selected.risk} risk</Badge>
                <Badge variant="secondary">
                  {selected.target} target range
                </Badge>
              </div>
              ...
              <p className="text-xs leading-5 text-muted-foreground">
                This demo signal is not a recommendation or guarantee. Validate
                market data and consult a qualified professional where
                appropriate.
              </p>
            </>
          )}
