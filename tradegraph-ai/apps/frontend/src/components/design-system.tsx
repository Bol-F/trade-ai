...
export function PageHeader({
  ...
  return (
    <header className="space-y-5 border-b pb-7">
      {breadcrumbs && (
...
export function FilterBar({
  ...
  return (
    <section
      ...
      className="sticky top-[65px] z-20 mt-6 rounded-xl border bg-background/95 shadow-sm backdrop-blur"
    >
      <div className="border-b px-4 py-3">
        <h2 id="filter-heading" className="text-sm font-medium">
          {title}
        </h2>
      </div>
  ...
  return (
    <Card className="gap-0 py-0 shadow-none">
      <CardContent className="p-5">
        ...
        <p
          className="mt-5 font-mono text-2xl font-semibold tracking-tight"
...
export function EmptyState({
  ...
  return (
    <div className="flex min-h-48 flex-col items-center justify-center rounded-lg border border-dashed p-8 text-center">
      <Database aria-hidden="true" className="size-6 text-muted-foreground" />
...
export function LoadingSkeleton({ rows = 3 }: { rows?: number }) {
  return (
    <div aria-label="Loading content" aria-busy="true" className="space-y-3">
...
export function LoadingState({ label = "Loading data…" }: { label?: string }) {
  return (
    <div
      role="status"
      aria-live="polite"
      className="flex min-h-32 items-center justify-center gap-3 rounded-xl border bg-card p-6 text-sm text-muted-foreground"
    >
      <LoaderCircle
        aria-hidden="true"
...
export function MethodologyTooltip({ text }: { text: string }) {
  return (
    <Tooltip>
      <TooltipTrigger aria-label="Methodology explanation">
...
export function RiskIndicator({ score }: { score: number }) {
  ...
  return (
    <div aria-label={`${label} exposure, ${score.toFixed(1)} out of 100`}>
  ...
  const tone =
    bounded >= 80
      ...
      : bounded >= 55
        ...
        : "[&_[data-slot=progress-indicator]]:bg-warning";
  return (
    <div aria-label={`${label}: ${level}, ${bounded.toFixed(0)} percent`}>
  ...
  return (
    <Badge
      ...
      className={cn(
        ...
        tone === "danger" &&
          "border-destructive/30 bg-danger-surface text-destructive",
