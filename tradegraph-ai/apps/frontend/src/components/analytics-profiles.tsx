...
function historyOption(
  points: { year: number; value: string | number | null }[],
): EChartsOption {
  return {
    ...
    yAxis: {
      type: "value",
      axisLabel: { formatter: (value: number) => money.format(value) },
    },
    series: [
      {
        type: "line",
...
export function CountryAnalyticsProfile({ iso3 }: { iso3: string }) {
  ...
  const profile = useQuery({
    queryKey: ["country-profile", normalizedIso3],
    queryFn: () => analyticsApi.countryProfile(normalizedIso3),
  });
  const catalog = useQuery({
    queryKey: queryKeys.country(normalizedIso3),
    queryFn: () => catalogApi.country(normalizedIso3),
  });
  ...
  return (
    <PageContainer className="py-8 sm:py-10">
      <PageHeader
        ...
        actions={
          <Link
            ...
            href={`/explorer?importer=${normalizedIso3}`}
          >
            Open in Explorer
          </Link>
        }
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <KpiCard
          label="Total imports"
          value={money.format(data.total_imports_usd)}
          detail="Across the active dataset"
        />
        <KpiCard
          label="Total exports"
          value={money.format(data.total_exports_usd)}
          detail="Across the active dataset"
        />
        <KpiCard
          label="Trade balance"
          value={money.format(balance)}
          detail={balance >= 0 ? "Export surplus" : "Import deficit"}
        />
        <KpiCard
          label="Supplier HHI"
          value={data.concentration.hhi.toFixed(3)}
          detail="Higher means more concentrated"
        />
        <KpiCard
          label="Exposure score"
          value={`${data.exposure.score.toFixed(1)} / 100`}
          detail="Composite supply risk"
        />
      </div>

      <div className="mt-6 grid gap-6 xl:grid-cols-[2fr_1fr]">
        <ChartCard
          title="Historical trade"
          description="Annual trade value in current US dollars."
          summary={chartSummary}
        >
          <EChart
            option={historyOption(data.history)}
            ariaLabel={chartSummary}
          />
        </ChartCard>
        <Card>
          <CardHeader>
            <CardTitle>Supply exposure</CardTitle>
          </CardHeader>
            ...
            <dl className="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <dd className="font-mono">
                  {data.exposure.volatility.toFixed(3)}
                </dd>
              </div>
              <div>
                <dt className="text-muted-foreground">Suppliers</dt>
                <dd className="font-mono">{data.exposure.supplier_count}</dd>
              </div>
              <div>
                <dt className="text-muted-foreground">HHI</dt>
                <dd className="font-mono">{data.exposure.hhi.toFixed(3)}</dd>
              </div>
              <div>
                <dt className="text-muted-foreground">Quantity data</dt>
                <dd>
                  {data.exposure.quantity_data_available
                    ? "Available"
                    : "Limited"}
                </dd>
              </div>
            </dl>
            {(data.exposure.insufficient_history ||
              !data.exposure.quantity_data_available) && (
              <p
                role="status"
                className="rounded-lg border border-warning/30 bg-warning-surface p-3 text-sm"
              >
                Interpret cautiously:{" "}
                {data.exposure.insufficient_history
                  ? "fewer than two annual observations. "
                  : ""}
                {!data.exposure.quantity_data_available
                  ? "Quantity history is incomplete."
                  : ""}
              </p>
            )}
          </CardContent>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        <Ranking
          ...
          rows={data.top_products.map((row) => ({
            label: `HS ${row.code}`,
            value: money.format(row.trade_value_usd),
            href: `/explorer?importer=${normalizedIso3}&product=${row.code}`,
          }))}
        />
        <Ranking
          title="Top suppliers"
          rows={data.top_suppliers.map((row) => ({
            label: `${row.name} (${row.iso3})`,
            value: money.format(row.trade_value_usd),
            href: `/explorer?importer=${normalizedIso3}&exporter=${row.iso3}`,
          }))}
        />
        <Ranking
          ...
          rows={data.top_destinations.map((row) => ({
            label: `${row.name} (${row.iso3})`,
            value: money.format(row.trade_value_usd),
            href: `/explorer?exporter=${normalizedIso3}&importer=${row.iso3}`,
          }))}
        />
      </div>
      <p className="mt-6 text-xs text-muted-foreground">
        {data.exposure.methodology}
      </p>
    </PageContainer>
  );
}
...
export function ProductAnalyticsProfile({ code }: { code: string }) {
  ...
  const profile = useQuery({
    queryKey: ["product-profile", hs2],
    queryFn: () => analyticsApi.productProfile(hs2),
  });
  const catalog = useQuery({
  ...
  return (
    <PageContainer className="py-8 sm:py-10">
      <PageHeader
        ...
        metadata={
          <>
            <Badge variant="outline">HS{level}</Badge>
            <DatasetVersionBadge version={meta.dataset_version} />
            <DataFreshnessBadge year={meta.source_period_end} />
          </>
        }
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        <KpiCard
          label="Supplier HHI"
          value={data.concentration.hhi.toFixed(3)}
          detail="0–1; higher means more concentrated"
        />
        <KpiCard
          label="Active exporters"
          value={String(data.concentration.supplier_count)}
        />
        <KpiCard
          label="Flagged years"
          value={String(flagged.length)}
          detail={flagged.length ? "Review anomalies" : "No elevated signal"}
        />
      </div>

      <div className="mt-6">
        <ChartCard
          title="Global trade trend"
          description="Annual trade value aggregated for this HS2 category."
          summary={chartSummary}
        >
          <EChart
            option={historyOption(data.global_trend)}
            ariaLabel={chartSummary}
          />
        </ChartCard>
      </div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        <Ranking
          title="Top exporters"
          rows={data.top_exporters.map((row) => ({
            label: `${row.name} (${row.iso3})`,
            value: money.format(row.trade_value_usd),
            href: `/explorer?exporter=${row.iso3}&product=${hs2}`,
          }))}
        />
        <Ranking
          title="Top importers"
          rows={data.top_importers.map((row) => ({
            label: `${row.name} (${row.iso3})`,
            value: money.format(row.trade_value_usd),
            href: `/explorer?importer=${row.iso3}&product=${hs2}`,
          }))}
        />
        <Ranking
          title="Fastest-growing exporters"
          rows={data.fastest_growing_countries.map((row) => ({
            label: row.iso3,
            value: percent.format(row.cagr),
            href: `/explorer?exporter=${row.iso3}&product=${hs2}`,
          }))}
        />
      </div>
      ...
      <Card className="mt-6">
        ...
        <CardContent>
          {flagged.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              No elevated annual anomaly signal was detected.
            </p>
          ) : (
  ...
  return (
    <PageContainer className="py-16">
      <ErrorState
        ...
        description="The profile could not be loaded. Check the active dataset and try again."
      />
    </PageContainer>
  );
}
