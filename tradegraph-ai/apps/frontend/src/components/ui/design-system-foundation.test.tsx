...
describe("core design-system controls", () => {
  ...
  it("supports labelled checkbox and switch interactions", () => {
    ...
    const checkbox = screen.getByRole("checkbox", { name: "Include delayed data" })
  ...
  it("provides a keyboard-accessible search clear action", () => {
    let cleared = false
    render(<SearchInput aria-label="Search instruments" value="energy" readOnly onClear={() => { cleared = true }} />)
    fireEvent.click(screen.getByRole("button", { name: "Clear search" }))
    expect(cleared).toBe(true)
  })

  it("renders modal title and description semantics", () => {
    render(<Dialog defaultOpen><DialogContent><DialogTitle>Confirm export</DialogTitle><DialogDescription>The report contains filtered trade data.</DialogDescription></DialogContent></Dialog>)
    expect(screen.getByRole("dialog", { name: "Confirm export" })).toHaveAccessibleDescription("The report contains filtered trade data.")
...
function ToastFixture() {
  ...
  return <Button onClick={() => toast({ tone: "success", title: "Analysis saved", description: "Available in your workspace." })}>Save</Button>
}
