...
const resourceLinks: readonly [TranslationKey, string][] = [
...
export function SiteHeader() {
  ...
  const moreIsActive =
    secondaryLinks.some(([, href]) => pathname.startsWith(href)) ||
    ...
    <header className="sticky top-0 z-50 border-b bg-background/95 backdrop-blur">
      <PageContainer className="flex min-h-16 items-center gap-3">
        ...
        >
          ...
          <span className="hidden sm:inline">TradeGraph AI</span>
        </Link>
        <nav
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                className={cn(
                  "gap-1 px-2.5 text-sm font-normal text-muted-foreground",
                  moreIsActive && "bg-muted font-medium text-foreground",
                )}
              >
                {t("nav.more")}
            ...
            <SheetContent side="right" className="w-[min(90vw,360px)] p-0">
              ...
              >
                ...
                {user?.role === "admin" && (
                  <>
                    <Separator className="my-2" />
                    <p className="px-2.5 text-xs font-medium uppercase tracking-wider text-muted-foreground">
                      {t("nav.administration")}
                    </p>
                    <SheetClose asChild>
                      <Link
                        href="/admin/data-health"
                        className={linkClass("/admin/data-health")}
                      >
                        {t("nav.dataHealth")}
                      </Link>
                    </SheetClose>
      ...
      {user?.role === "admin" && (
        <div className="hidden border-t bg-muted/35 xl:block">
          <PageContainer className="flex h-9 items-center gap-3 text-xs">
