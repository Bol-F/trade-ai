...
export function MapDashboard() {
  ...
  const filters = { start_year: applied.year, end_year: applied.year, product: applied.product, top: applied.top }
  ...
  return <PageContainer className="py-10">
    ...
    <FilterBar><form onSubmit={apply}><FilterSection className="sm:grid-cols-3 xl:grid-cols-3">
      <Label>Selected year<Input className="mt-2" type="number" min="1900" max="2100" required value={draft.year} onChange={(event) => setDraft(value => ({ ...value, year: event.target.value }))} /></Label>
      <Label>HS2 product<Input className="mt-2" inputMode="numeric" maxLength={2} placeholder="All products" value={draft.product} onChange={(event) => setDraft(value => ({ ...value, product: event.target.value.replace(/\D/g, "").slice(0, 2) }))} /></Label>
      <Label>Visible flows<Select className="mt-2" value={draft.top} onChange={(event) => setDraft(value => ({ ...value, top: event.target.value }))}><option value="10">Top 10</option><option value="25">Top 25</option><option value="50">Top 50</option></Select></Label>
    </FilterSection><div className="mt-4"><Button type="submit" disabled={query.isFetching}>{query.isFetching ? "Updating map…" : "Apply filters"}</Button></div></form></FilterBar>
