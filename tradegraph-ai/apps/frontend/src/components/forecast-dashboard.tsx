...
export function ForecastDashboard() {
  ...
  return <PageContainer className="py-10">
    ...
    {result && option && <>
      ...
      {result.warnings.map(warning => <div role="status" key={warning.code} className="mt-3 rounded-lg border border-warning/30 bg-warning-surface p-3 text-sm"><strong>{warning.code.replaceAll("_", " ")}:</strong> {warning.message}</div>)}
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        ...
        <Card className="border-warning/30 bg-warning-surface shadow-none"><CardHeader><CardTitle className="flex items-center gap-2"><AlertTriangle className="size-4 text-warning" />Uncertainty and limitations</CardTitle></CardHeader><CardContent className="text-sm leading-6 text-muted-foreground">The interval is estimated from validation residuals and cannot anticipate policy changes, conflict, weather, reporting revisions, or structural breaks. Compare the forecast with the baseline and investigate the historical lane.</CardContent></Card>
      </div>
